...
public class ExpertTest {
	...
	public static void main(String[] args) {
		...
		if(com.isCreateSuccess()){
			...
			System.out.println("hwnd result " + hwnd);
			if(hwnd>0){	
				//如果找到了记事本这个窗口了，确定是有这个程序存在，那么可以继续以下操作,因为没有试用Com.bind所以操作的只是前台我们看到的，模拟我们的操作
				hwnd=window.getForegroundWindow();
				System.out.println("hwnd result " + hwnd);
				int fhwnd = window.getForegroundFocus();		
				System.out.println("fhwnd result " + fhwnd);
				sleep.sleep(3000);
				boolean b = window.sendString(fhwnd, "111111哈哈");
				System.out.println("sendString result " + b);
				String spot = mouse.getCursorSpot();
				System.out.println("spot result " + spot);
				/*Mouse mouse=new Mouse(com.getActiveXComponent());	//鼠标模拟操作类
